...
locals {
  ...
  playbook_dir = join("/", [local.user_dir, "ansible", "playbooks"])
  etc_iron_dir = join("/", ["", "etc", "iron"])
  ...
  haproxy_ip_addresses = join("\n", [for k, v in var.haproxy_instances : format("%s", v.private_ip)])
...
resource "aws_instance" "ec2_ansible" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mkdir -p ~/.ssh",
      "chmod 700 ~/.ssh",
      "sudo mkdir -p ${local.etc_iron_dir}",
      "sudo chown ${var.ami_user} ${local.etc_iron_dir}",
      "sudo chmod 755 ${local.etc_iron_dir}",
    ]
  ...
  provisioner "file" { # k8s nodes list
    ...
    destination = join("/", [local.etc_iron_dir, local.k8s_nodes_list_filename])
  ...
  provisioner "file" { # k8s nodes list
    ...
    destination = join("/", [local.etc_iron_dir, "haproxy_nodes.txt"])
  ...
  provisioner "file" { # k8s nodes list
    content     = local.haproxy_ip_addresses
    destination = join("/", [ local.etc_iron_dir, "haproxy_ip_addresses.txt"])
  ...
  provisioner "file" { # ansible playbooks
    ...
    destination = local.playbook_dir # join("/", [, "kube_install_nodes.yml"])
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # "while IFS= read -r hostname; do dig +short -4 $hostname || true; done < /etc/iron/k8s_nodes.txt >> /etc/iron/k8s_ip_addresses.txt",
      # "ssh-keyscan -H -p 22 -f /etc/iron/k8s_nodes.txt >> ~/.ssh/known_hosts",
      # "ssh-keyscan -H -p 22 -f /etc/iron/haproxy_nodes.txt >> ~/.ssh/known_hosts",
      # "sudo chown -R root /etc/iron",
    ]
